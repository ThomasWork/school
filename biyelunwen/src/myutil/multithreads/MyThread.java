...
public abstract class MyThread implements Runnable
{
		...
		for(int i=0; i<total; i++)
		{
			if(MyThread.showThreadProcess)
			{
				if(i%MyThread.showLoop==0)
					System.out.println("任务"+this.id+"完成:"+NumberUtil.df4.format(i*100.0/total)+"%");
			}
	...
	{		
		...
		System.out.println("当前已经完成:"+finish+"；一组:"+MyThread.stageNumber+"；显示:"+showLoop);
		...
		while(finish<urls.size())
		{
			...
			{
				System.out.println("总体进度:"+finish+"("+NumberUtil.df4.format(finish*100.0/totalNum)+"%)");
			}
