...
public class GeoAreaGroup
{
	...
	public static void getUserRoutes() {
		...
		for (User u: users) {
					...
					if (gap.contains(p)) {
						...
						if (places.get(places.size() - 1).equals(place) == false) {
							/*if (places.contains(place)) {
								System.out.println(u.id + "find: " + place);
								lines.add(StringUtil.join(places, ","));
								places.clear();
								places.add(place);
							} else {*/
								places.add(place);
			...
			String list = u.id + "," + StringUtil.join(tags, ",");
			if (list.length() > 0) {
				System.out.println(list);
				lines.add(list);
			}
			if (u.id.equals("90577814@N00")) {
				u.drawRoutes();
			}
		}
